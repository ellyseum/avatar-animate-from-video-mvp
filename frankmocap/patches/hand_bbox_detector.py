...
class Third_View_Detector(BodyPoseEstimator):
    ...
    def __get_raw_hand_bbox(self, img):
        instances = self.hand_detector(img)['instances']
        bbox_tensor = instances.pred_boxes
    ...
    def detect_hand_bbox(self, img):
        ...
        # get raw hand bboxes (now includes confidence scores)
        raw_hand_bboxes, raw_hand_scores = self.__get_raw_hand_bbox(img)
        ...
        if num_bbox > 0:
            for idx, body_pose in enumerate(body_pose_list):
                ...
                if dist_left_arm[left_id] < float('inf'):
                    ...
                    hand_bboxes['left_hand_score'] = float(raw_hand_scores[left_id])
                if dist_right_arm[right_id] < float('inf'):
                    hand_bboxes['right_hand'] = raw_hand_bboxes[right_id].copy()
                    hand_bboxes['right_hand_score'] = float(raw_hand_scores[right_id])
    ...
    def detect_hand_bbox(self, img):
        ...
        if bboxes is not None:
            ...
            if len(left_bbox)>0:
                ...
                hand_bbox_list[0]['left_hand_score'] = 1.0  # ego detector doesn't expose scores

            right_bbox = bboxes[hand_types==1]
            if len(right_bbox)>0:
                hand_bbox_list[0]['right_hand'] = right_bbox[0]
                hand_bbox_list[0]['right_hand_score'] = 1.0
...
class HandBboxDetector(object):
    def __init__(self, view_type, device):
        ...
        self.last_hand_scores = []  # confidence scores from last detect_hand_bbox call
    ...
    def detect_hand_bbox(self, img_bgr):
        ...
        # Extract confidence scores from hand_bbox dicts BEFORE downstream
        # consumers iterate over keys (scores would break x0,y0,w,h unpacking).
        # Scores are stored on self.last_hand_scores for callers that need them.
        self.last_hand_scores = []
        for hand_bbox in hand_bbox_list:
            scores = {'left_hand_score': 0.0, 'right_hand_score': 0.0}
            if hand_bbox is not None:
                scores['left_hand_score'] = hand_bbox.pop('left_hand_score', 0.0)
                scores['right_hand_score'] = hand_bbox.pop('right_hand_score', 0.0)
            self.last_hand_scores.append(scores)
